...
void optimize_fs_method(
	...
	for (int i=0; i<i_num_folds; i++) {
		...
		for (int j=0; j<6; j++) {
			...
			// get maximum score
			float f_max_score = vec3_scores[i][j][0];
			// compute median abs deviation
			float f_median = vec3_scores[i][j][i_num_features/2-1];
			vector<float> vec_abs_devs(i_num_features, 0);
			for (int k=0; k<i_num_features; k++)
				vec_abs_devs[k] = fabs(vec3_scores[i][j][k]-f_median);
			quicksort(vec_abs_devs);
			float f_mad = vec_abs_devs[i_num_features/2-1];
			// compute distance
			for (int k=0; k<i_num_features; k++) {
				float f_diff = vec3_scores[i][j][k]-f_max_score;
				vec_sums[vec3_feature_index[i][j][k]] += f_diff*f_diff/(f_mad*f_mad);
			}
		}
		quicksort_i(vec_sums, vec2_opt_feature_index[i], SORT_ASCENDING);
	}
}
...
int main(int argc, char* argv[]) {
	...
	combine_samples(data_scomb_x, data_scomb_y, data_x, data_y);
	index_samples(&data_scomb_y, vec2_scomb_index_class);
	vec_scomb_index = vector<int>(data_scomb_y.size(), 0);
	Matrix<vector<float> > data_scomb_rankprod_precompute;
	//rankprod_precompute(data_scomb_x, vec2_scomb_index_class, data_scomb_rankprod_precompute);
	...
	for (int i=0; i<i_num_iterations; i++) {
		...
		combine_data(vec2_scomb_folds_train, vec2_scomb_folds_test, data_x, data_y, vec3_folds_train, vec3_folds_test);
		...
		optimize_fs_method(
			...
			data_scomb_rankprod_precompute
		);
		...
		vector<vector<float> > vec2_rcomb_average_ranks(i_num_folds, vector<float>(i_num_features, 0));
		vector<vector<int> > vec2_rcomb_feature_index(i_num_folds);
		for (int fold=0; fold<i_num_folds; fold++) {
			for (int ds=0; ds<i_num_datasets; ds++)
				for (int fs=0; fs<i_num_features; fs++)
					vec2_rcomb_average_ranks[fold][fs] += vec3_opt_feature_ranks[ds][fold][fs];
