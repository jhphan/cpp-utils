...
void select_features(int i_fs_method, Matrix<float>& mat_data, vector<float>& vec_data_y, vector<int>& vec_index, vector<vector<int> >& vec2_index_class, vector<int>& vec_feature_index, int i_max_features, Matrix<vector<float> >& mat_scores) {
	...
	for (int i=0; i<i_num_iterations; i++) {
		...
		for (int fold=0; fold<i_num_folds; fold++) {
			for (int fs=0; fs<i_num_features; fs++) {
				for (int ds=0; ds<i_num_datasets; ds++)
					vec2_rcomb_average_ranks[fold][fs] *= (vec3_opt_feature_ranks[ds][fold][fs]+1)/(float)i_num_features;
