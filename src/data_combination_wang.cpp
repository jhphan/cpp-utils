...
void fs_method_single(
	...
	for (int i=0; i<i_num_folds; i++) {
		...
		wang_index(data_train_x[i], vec2_train_index[i], vec2_train_index_class, vec_scores, vec2_opt_feature_index[i]);
	}
}
...
int main(int argc, char* argv[]) {
	...
	for (int i=0; i<i_num_iterations; i++) {
		...
		// 6)
		// -- rank-combine features, wang method
		...
		for (int fold=0; fold<i_num_folds; fold++) {
			...
			wang_index(tmp_vec_mat_data, tmp_vec2_index, tmp_vec3_index_class, tmp_vec_scores, vec2_rcomb_feature_index[fold]);
		}
