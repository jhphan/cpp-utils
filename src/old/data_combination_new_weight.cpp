...
void optimize_fs_method(
		...
		vector<Matrix<float> >& data_test_x,
	...
	vector<vector<vector<int> > > vec3_feature_index(i_num_folds, vector<vector<int> >(5));
	vector<vector<vector<float> > > vec3_perf(i_num_folds, vector<vector<float> >(5, vector<float>(i_max_features, 0)));
	...
	for (int i=0; i<i_num_iterations; i++) {
		...
		for (int j=0; j<i_num_folds; j++) {
			for (int ds=0; ds<i_num_datasets; ds++) {
				...
				quantile_reference(data_x[ds], vec3_folds_train[ds][j], quant_ref);
				// quantile normalize the training data to the reference
				quantile_normalize_matrix(data_x[ds], norm_data_x[ds][j], vec3_folds_train[ds][j], quant_ref);
				//norm_data_x[ds][j] = data_x[ds];
				// quantile normalize the validation data to each individual dataset
				quantile_normalize_matrix(data_val_x, norm_data_val_x[ds][j], vec_val_index, quant_ref);
				//norm_data_val_x[ds][j] = data_val_x;
				// quantile normalize the test data
				quantile_normalize_matrix(data_x[ds], norm_data_test_x[ds][j], vec3_folds_test[ds][j], quant_ref);
				//norm_data_test_x[ds][j] = data_x[ds];
			}
			vector<float> scomb_quant_ref;
			// create a quantile reference for the training portion of combined data
			quantile_reference(data_scomb_x, vec2_scomb_folds_train[j], scomb_quant_ref);
			// quantile normalize the training data
			quantile_normalize_matrix(data_scomb_x, norm_data_scomb_x[j], vec2_scomb_folds_train[j], scomb_quant_ref);
			//norm_data_scomb_x[j] = data_scomb_x;
			// quantile normalize validation data to scomb data
			quantile_normalize_matrix(data_val_x, norm_data_scomb_val_x[j], vec_val_index, scomb_quant_ref);
			//norm_data_scomb_val_x[j] = data_val_x;
			// quantile normalize the test data
			quantile_normalize_matrix(data_scomb_x, norm_data_scomb_test_x[j], vec2_scomb_folds_test[j], scomb_quant_ref);
			//norm_data_scomb_test_x[j] = data_scomb_x;
		}
		...
		optimize_fs_method(
			...
			norm_data_scomb_test_x,
			data_scomb_y,
			vec2_scomb_folds_test,
			vec2_scomb_index_class,
			i_max_features,
			i_cls_method,
			i_num_folds,
		...
		vector<vector<vector<int> > > vec3_opt_feature_ranks(i_num_datasets, vector<vector<int> >(i_num_folds));
		for (int ds=0; ds<i_num_datasets; ds++)
			for (int fold=0; fold<i_num_folds; fold++) {
				compute_ranks(vec3_opt_feature_index[ds][fold], vec3_opt_feature_ranks[ds][fold]);
				//for (int k=0; k<50; k++) {
				//	cerr << vec3_opt_feature_index[ds][fold][k] << "|" << vec3_opt_feature_ranks[ds][fold][k] << ",";
				//}
				//cerr << endl;
			}
		vector<vector<float> > vec2_rcomb_average_ranks(i_num_folds, vector<float>(i_num_features, 1));
		vector<vector<int> > vec2_rcomb_feature_index(i_num_folds);
		for (int fold=0; fold<i_num_folds; fold++) {
			for (int fs=0; fs<i_num_features; fs++) {
				for (int ds=0; ds<i_num_datasets; ds++)
					vec2_rcomb_average_ranks[fold][fs] += 1/(vec3_opt_feature_ranks[ds][fold][fs]+1);
				vec2_rcomb_average_ranks[fold][fs] = 1/vec2_rcomb_average_ranks[fold][fs];
			}
			quicksort_i(vec2_rcomb_average_ranks[fold], vec2_rcomb_feature_index[fold], SORT_ASCENDING);
		}
