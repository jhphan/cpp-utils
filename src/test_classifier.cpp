...
int main(int argc, char* argv[]) {
	...
	fold_change_index(x, vec2_folds_train[0], vec2_index_class, vec_scores, vec_feature_index);
	//mrmr_index(x, y, vec2_folds_train[0], vec_scores, vec_feature_index, 10, true);
	...
	int i_max_fs = 10;
	for (int i=1; i<=i_max_fs; i++) {
		cout << "feature size: " << i << endl;

		Matrix<float> sub_x;
		extract_features(x, vec_feature_index, i, sub_x);
		for (int j=0; j<i_num_samples; j++) {
			cout << y[j] << " ";
		}
		cout << endl;
		cout << sub_x << endl << endl;
		Classifier* cls = new LR();
		cls->train(&sub_x, &y, vec2_folds_train[0]);
		vector<float> vec_dec_vals;
		cls->predict_points_in_matrix_scaled(&sub_x, vec2_folds_test[0], vec_dec_vals);

		//cout << "size dv: " << vec_dec_vals.size() << endl;
